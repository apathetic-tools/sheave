...
either the installed package (src/sheave) or the standalone single-file script
(dist/sheave.py) based on the RUNTIME_MODE environment variable.

Verifies:
  - When RUNTIME_MODE=singlefile: All modules resolve to dist/sheave.py
  - When RUNTIME_MODE is unset (installed): All modules resolve to src/sheave/
...
def test_pytest_runtime_cache_integrity() -> None:  # noqa: PLR0912
    ...
    if mode == "singlefile":
        ...
        # standalone mode correctly via sys.modules.get("sheave")
